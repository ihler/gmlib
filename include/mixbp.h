#ifndef __MEX_MIXBP_H
#define __MEX_MIXBP_H
...
// Factor graph algorithm specialization for "mixed-product" loopy belief propagation
//   for Marginal MAP estimation tasks

class mixbp : public gmAlg, public factorGraph, virtual public mxObject {
public:
...
public:
  ...
  mixbp()                                 : factorGraph()    { setProperties(); _elimType.resize(nvar(),ElimType::Sum); }
  mixbp(const factorGraph& fg)            : factorGraph(fg)  { setProperties(); _elimType.resize(nvar(),ElimType::Sum); }
  mixbp(vector<Factor> fs)                : factorGraph(fs)  { setProperties(); _elimType.resize(nvar(),ElimType::Sum); }
  template <class InputIterator>
  mixbp(InputIterator f, InputIterator l) : factorGraph(f,l) { setProperties(); _elimType.resize(nvar()); setElimType(ElimType::Sum); }

  virtual mixbp* clone() const            { mixbp* fg = new mixbp(*this); return fg; }
  ...
  void        mxSwap(mixbp& gm);     // exchange with another object and matlab identity
  /////////////////////////////////////////////////////////////////////////////////////////
#endif
  ...
  double logZlb() const { throw std::runtime_error("Not available"); }
	...
	// Algorithm choices: basic "mixed-product" updates, or proximal sum-product method	
	MEX_ENUM( Method , Sum,Max,Mixed,Proximal );
	Method _method;

	// Set desired variable types (max variables vs sum variables)
	MEX_ENUM( ElimType , Sum,Max ); 
	mex::vector<ElimType> _elimType;
  ...
  mex::vector<VarSet> _varsMax, _varsSum;  // factor-by-factor max/sum variable split
  mex::vector<Factor> _proximal;           // proximal terms for each factor (if used)
  double _epsilon;                         // temperature for annealed methods
  void setEpsilon(double ep) {_epsilon=ep; } 

  MEX_ENUM( Schedule , Fixed,Random,Flood,Priority );

  MEX_ENUM( Property , Method,Schedule,Distance,StopIter,StopObj,StopMsg,StopTime );
  virtual void setProperties(std::string opt=std::string()) {
    if (opt.length()==0) {
      setProperties("Method=Proximal,Schedule=Priority,Distance=HPM,StopIter=10,StopObj=-1,StopMsg=-1,StopTime=-1");
      return;
    }
    ...
    for (size_t i=0;i<strs.size();++i) {
      ...
      switch( Property(asgn[0].c_str()) ) {
        case Property::Method:    _method  = Method(asgn[1].c_str()); break;
  ...
  virtual void init() { 
    ...
    setEpsilon(.01);
    _varsMax=vector<VarSet>(_factors.size());                     // figure out max/sum vars for each factor
    _varsSum=vector<VarSet>(_factors.size());                     //  (these can get re-used a lot)
    for (size_t f=0;f<_factors.size();++f) {
      for (VarSet::const_iterator v=_factors[f].vars().begin();v!=_factors[f].vars().end();++v)
        if (_elimType[_vindex(*v)]==ElimType::Max) _varsMax[f]+=*v; else _varsSum[f]+=*v; 
    }
...
#ifndef USE_LOG
#error Messages assumed to be log
#endif
    if (belief(n).nvar()==1) updateOutgoingVar(n);     // if a variable node, do this
    else                     updateOutgoingFactor(n);  // otherwise do this one
  }

  void updateOutgoingVar(size_t n) {
    const set<EdgeID>& nbrs = neighbors(n);        // get the list of neighbors
    Factor logBel = log(belief(n));
    if (_method==Method::Proximal && _elimType[_vindex(belief(n).vars()[0])]==ElimType::Max )
      logBel += log(_proximal[n]);                // if proximal update, add proximal model change
    for (set<EdgeID>::const_iterator i=nbrs.begin();i!=nbrs.end();++i) {
      _msgNew[i->idx] = logBel - _msg[i->ridx];   // no elimination: single variable
      _msgNew[i->idx] -= _msgNew[i->idx].max();   // normalize message
      if (_sched==Schedule::Priority)             // and update priority in schedule
        priority.insert( exp(_msgNew[i->idx]).distance(exp(_msg[i->idx]),_dist) , i->idx );  // !!! fix faster
    }
  }

  void updateOutgoingFactor(size_t n) {
    const set<EdgeID>& nbrs = neighbors(n);        // get the list of neighbors
    Factor logBel = log(belief(n));
    Factor logBel0(0.0); 

    if (_method != Method::Proximal) {
      // if any max variables, compute their marginal, and annealed argmax operator:
      if (_varsMax[n].size()) {   // only if sumVar also???
        logBel0 = logBel.logsumexp( _varsSum[n] );             // find marginal over max vars
        logBel  += (logBel0 - logBel0.max())*(1/_epsilon);    // and annealed argmax over max vars
      }
    }
    // if proximal & >1 max var, add proximal model terms (should be size of logBel0) 
    if (_method==Method::Proximal && _varsMax[n].size() > 1) logBel += log(_proximal[n]);
   
    // now compute outgoing messages:
    for (set<EdgeID>::const_iterator i=nbrs.begin();i!=nbrs.end();++i) {
      Var v = belief(i->second).vars()[0];
      if (_method!=Method::Proximal && _elimType[_vindex(v)]==ElimType::Max) 
        _msgNew[i->idx] = (logBel0-_msg[i->ridx]).maxmarginal( v );  // !!! epsilon elim? 
      else
        _msgNew[i->idx] = (logBel-_msg[i->ridx]).logsumexp( belief(n).vars() - belief(i->second).vars() );
 ...
 void updateProximal() {
   if (_proximal.size()==0) _proximal.resize( _factors.size() ); // allocate for factors if necessary
   for (size_t f=0;f<_factors.size();++f) {
     if (_varsMax[f].size() > 0) {
       Factor logBel = log(belief(f));
       _proximal[f] = logBel.logsumexp( _varsSum[f] );            // vars: remove entropy term for max vars
       if (_varsMax[f].size() > 1) {                              // factors: remove MI terms for max vars
         for (VarSet::const_iterator v=_varsMax[f].begin();v!=_varsMax[f].end();++v)
           _proximal[f] -= log(belief( _vindex(*v) ));
       }
     } else {
      _proximal[f] = Factor(0.0);                              // blank factor if no max vars
     }
   }
   for (size_t e=0;e<2*nEdges();++e)                           //  reset all edges to infinity
     if (edge(e)!=EdgeID::NO_EDGE) priority.insert( std::numeric_limits<double>::infinity() , e);
 }
 void outerStepAnnealed() {
   // epsilon = 1/step
 }
...
(1) Annealed: 
@ variable node: bi = product of incoming messages
  Msgs to factors : bi / msg_in
@ factor node:
  Compute belief b_a, 
  Compute normalized max-node belief b_a0 by marginalizing sum nodes & divide by max value
  Msgs to max nodes: max-marginalize or epsilon-marginalize (b_a0 / msg_in)
  Msgs to sum nodes: marginalize [ sigma(b_a0) * b_a / msg_in ]


(2) Proximal:
@ variable node: bi = product of incoming messages 
  Msgs to factors : bi * bi_prox_old / msg_in            (ALT: bi^2 / msg_in, as immediate prox update)
@ factor node: regular sum-product:
  Compute belief b_a, 
  Msgs to nodes: marginalize [ b_a * ba_prox_old / msg_in ]
@ Outer loop: update bX_prox_old = bX * bX_prox_old for all i,a  (a: several max nodes, sum out others)


Different versions to try:
 1) Double-loop proximal: regular sum-product with "extra" multiply; outer loop updates bProx, all priorities -> inf
 2) Double-loop annealed: annealed messages; outer loop updates epsilon, all priorities -> inf
 3) Scheduled 1&2?  As before but keep track of which bProx / epsilons are worth revisiting?
 4) ""
 5) Immediate proximal: use current belief as bProx
 6) Fixed epsilon = .1 or .01   (=> free energy estimate is wrong?)
 7) Fixed epsilon = 0  (how implemented? zero-temp mixbp)
 8) Double schedule: prioritize message acceptance, belief (prox) acceptance
 9) Double schedule: prioritize message accept, epsilon changes (how?)
...
// !!! TODO: Mixed BP update:
/*
(1) if var node, just do nothing (?)
(2) split vars into max & sum sets
(3) belief(n) = product of factor & incoming messages (already updated?)
if some max:
 (4) compute belMax = \sum_{S} belief(n)
 (5) outgoing messages to max nodes (nodes with no sum vars): (logBel - msg[i->ridx]).max( others )
 if no sum vars, return
 (6) Compute sig = (belMax/belMax.max())^(1/eps); sig=sig/sig.sum(); belSum=belief(n)*sig;
(7) outgoing messages to sum nodes: (logBelSum - msg[i->ridx].sum( others )
*/
...
//////////////////////////////////////////////////////////////////////////////////////////////
bool mixbp::mxCheckValid(const mxArray* GM) { 
...
void mixbp::mxRelease() {  throw std::runtime_error("NOT IMPLEMENTED"); }
void mixbp::mxDestroy() { throw std::runtime_error("NOT IMPLEMENTED"); }

void mixbp::mxSwap(mixbp& gm) {
