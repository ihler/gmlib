...
public:
  ...
  superindex(const VarSet& full, const VarSet& sub, size_t offSub) {
    assert( full >> sub );
    Ns = sub.nvar();
    start_ = 0; //end_ = full.nrStates();            // initially, span the whole table
    const vsize* dimf = full.dims();               // get pointer to (all) variable dimensions (# states)
    dims = sub.dims();                             // keep pointer to shared (=subset) var dimensions
    state= new vsize[Ns];                          // allocate vectors for storing current state &
    add  = new vsize[Ns];                          //   statistics used to increment the subset index
    size_t i,j,d=1;
    for (i=0,j=0;i<full.nvar();++i) {
      if (j<sub.nvar() ? full[i]==sub[j] : false) {   // on variables in "sub", figure out how 
        ...
        add[j] = d;                     // (incrementing this variable will add "d" to the offset)
        j++;
        ...
        start_ += d*rem;                  //  & locate that value in full var set
      }
      d *= dimf[i];                    // (stepping over the next variable increments by (# states) times more
    }
    idx_ = start_;
    end_ = Ns ? add[Ns-1]*dims[Ns-1]+start_ : start_+1;  // end at (++last), one increment beyond the last entry
  }

  superindex(const superindex& S) {
    dims = S.dims; idx_=S.idx_; end_=S.end_; start_=S.start_;
    Ns = S.Ns;
  ...
  ~superindex(void) {
   ...
   superindex& operator++ (void) {
     if (Ns==0) { idx_=end_; return *this; }      // Empty variables = done on increment
