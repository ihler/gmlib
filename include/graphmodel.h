...
public:
  ...
  vector<vindex> pseudoTree(const VarOrder&, bool isAndOr=false) const; // find parents for given elim order
  ...
  // TODO: pass in container, make template?
  ...
  template <class MapType>
  inline double logP( const MapType& config ) {     // TODO: rename "logValue"
    double val = 0.0;
