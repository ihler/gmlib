...
// Graphical Model Algorithm: Generalized (Dual) Decomposition
// 

class gdd : public graphModel, public gmAlg, virtual public mxObject {
...
public:
  ...
  typedef graphModel::flist     	flist;        // collection of factor indices
	...
	typedef vector<index> 					matchList;
	typedef vector<index> 					nodeList;

public:
  gdd() : graphModel() { setProperties(); }
  gdd(vector<Factor> fs) : graphModel(fs) { setProperties(); }
  gdd(const graphModel& gm) : graphModel(gm) { setProperties(); }
  virtual gdd* clone() const { gdd* gm = new gdd(*this); return gm; }
...
std::pair<double,double> wmbe::sampleMixture(MapType& config) {
  ...
  for (int b=_nodes.size()-1;b>=0;--b) {
    ...
    for (nodeList::const_iterator ni=_nodes[b].begin();ni!=_nodes[b].end();++ni) {
      ...
      qComponent -= qComponent.max();
      qComponent *= (1.0 / node(*ni).weight);
      qComponent.exp();
      if (qComponent.sum() == 0.0) qComponent += 1.0/qComponent.nrStates();
      qMix += node(*ni).weight * qComponent;
    }
    qMix /= qMix.sum();
    size_t tmp = qMix.sample();
    config[ qMix.vars()[0] ] = tmp; //qMix.sample();
    logQx += std::log( qMix[ tmp ] ); //config[ qMix.vars()[0] ] ] );
    done |= qMix.vars();
  }
  logPx = logP( config );
