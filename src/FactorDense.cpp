...
Factor::value Factor::logEntropy(void) const {                             // compute entropy (in nats) of log-prob table
  value H=0;
	value lnZ = logsumexp();
  for (size_t i=0;i<nrStates();i++) {
		if ( t_[i] != -infty() ) H += std::exp(t_[i]-lnZ) * (t_[i]-lnZ);
  }
  return H;
}
...
size_t Factor::sample() const {
  double x=0.0, z = sum();
  //if (z==0.0) { std::cout<<"Sample non-normalizable: "<<*this<<"\n"; }
