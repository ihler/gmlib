...
vector<graphModel::vindex> graphModel::pseudoTree(const VarOrder& order, bool isAndOr) const {
  ...
  for (VarOrder::const_iterator i=order.begin(); i!=order.end(); ++i) {
    ...
    for (VarSet::const_iterator j=vi.begin(); j!=vi.end(); ++j) {
      ...
      for (VarOrder::const_iterator k=i; k!=order.end();++k) {
			...
			if (i + 1 != order.end())
				parents[*i] = *(i + 1);	// optional: force or-chain pseudoTree
    }
  }
  return parents;
}
...
//
// TODO: Needs MMAP support / prioritization of some variables...
